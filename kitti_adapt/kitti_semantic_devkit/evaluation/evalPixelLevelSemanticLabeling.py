...
if "KITTI_DATASET" in os.environ:
    ...
    config.kittiPath = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "..", ".."
    )

# Parameters that should be modified by user
config.exportFile = os.path.join(
    config.kittiPath, "evaluationResults", "resultPixelLevelSemanticLabeling.json"
)
config.groundTruthSearch = os.path.join(
    config.kittiPath, "training", "semantic", "*.png"
)
...
def getIouScoreForLabel(label, confMatrix, config):
    ...
    notIgnored = [
        l for l in config.evalLabels if not id2label[l].ignoreInEval and not l == label
    ]
...
def getIouScoreForCategory(category, confMatrix, config):
    ...
    labelIds = [
        label.id
        for label in labels
        if not label.ignoreInEval and label.id in config.evalLabels
    ]
...
def printConfMatrix(confMatrix, config):
    ...
    for label in config.evalLabels:
        print(
            "\b{text:{fill}>{width}}".format(
                width=config.printRow + 2, fill="-", text=" "
            ),
            end=" ",
        )
    print(
        "\b{text:{fill}>{width}}".format(width=config.printRow + 3, fill="-", text=" ")
    )
    ...
    for label in config.evalLabels:
        print(
            "\b{text:^{width}} |".format(
                width=config.printRow, text=id2label[label].name[0]
            ),
            end=" ",
        )
    ...
    for x in range(0, confMatrix.shape[0]):
        ...
        for y in range(0, len(confMatrix[x])):
            ...
            print(
                getColorEntry(matrixFieldValue, config)
                + "\b{text:>{width}.2f}  ".format(
                    width=config.printRow, text=matrixFieldValue
                )
                + config.nocol,
                end=" ",
            )
...
def printClassScores(scoreList, instScoreList, config):
    ...
    for label in config.evalLabels:
        ...
        iouStr = (
            getColorEntry(scoreList[labelName], config)
            + "{val:>5.3f}".format(val=scoreList[labelName])
            + config.nocol
        )
        niouStr = (
            getColorEntry(instScoreList[labelName], config)
...
def printCategoryScores(scoreDict, instScoreDict, config):
    ...
    for categoryName in scoreDict:
        if all(label.ignoreInEval for label in category2labels[categoryName]):
            continue
        iouStr = (
            getColorEntry(scoreDict[categoryName], config)
            + "{val:>5.3f}".format(val=scoreDict[categoryName])
            + config.nocol
        )
        niouStr = (
            getColorEntry(instScoreDict[categoryName], config)
...
def evaluateImgLists(predictionImgList, groundTruthImgList, config):
    ...
    for label in config.evalLabels:
        ...
        classInstScoreList[labelName] = getInstanceIouScoreForLabel(
            label, confMatrix, instStats, config
        )
    ...
    if not config.quiet:
        ...
        iouAvgStr = (
            getColorEntry(getScoreAverage(classScoreList, config), config)
            + "{avg:5.3f}".format(avg=getScoreAverage(classScoreList, config))
            + config.nocol
        )
        niouAvgStr = (
            getColorEntry(getScoreAverage(classInstScoreList, config), config)
            + "{avg:5.3f}".format(avg=getScoreAverage(classInstScoreList, config))
            + config.nocol
        )
        print("--------------------------------")
    ...
    for category in category2labels.keys():
        categoryInstScoreList[category] = getInstanceIouScoreForCategory(
            category, confMatrix, instStats, config
        )
    ...
    if not config.quiet:
        ...
        iouAvgStr = (
            getColorEntry(getScoreAverage(categoryScoreList, config), config)
            + "{avg:5.3f}".format(avg=getScoreAverage(categoryScoreList, config))
            + config.nocol
        )
        niouAvgStr = (
            getColorEntry(getScoreAverage(categoryInstScoreList, config), config)
            + "{avg:5.3f}".format(avg=getScoreAverage(categoryInstScoreList, config))
            + config.nocol
        )
        print("--------------------------------")
        print("Score Average : " + iouAvgStr + "    " + niouAvgStr)
...
def evaluatePair(
    ...
    if CSUPPORT:
        ...
        confMatrix = addToConfusionMatrix.cEvaluatePair(
            predictionNp, groundTruthNp, confMatrix, config.evalLabels
        )
    else:
    ...
    if config.evalPixelAccuracy:
        notIgnoredLabels = [
            l for l in config.evalLabels if not id2label[l].ignoreInEval
        ]
        ...
        perImageStats[predictionImgFileName]["nbCorrectPixels"] = np.count_nonzero(
            erroneousPixels
        )
...
def main(argv):
    ...
    if len(argv) >= 1:
        ...
        config.exportFile = os.path.join(
            config.predictionPath, "resultPixelLevelSemanticLabeling.json"
        )
        if len(argv) == 2:
        ...
        printError(
            "Wrong number of input arguments. Please input the path to the result folder."
        )
    return
