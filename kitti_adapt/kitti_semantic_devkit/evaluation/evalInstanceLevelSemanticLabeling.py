...
config.exportFile = os.path.join(
    config.kittiPath, "evaluationResults", "resultInstanceLevelSemanticLabeling.json"
)
config.groundTruthSearch = os.path.join(
    config.kittiPath, "training", "instance", "*.png"
)

# overlaps for evaluation
config.overlaps = np.arange(0.5, 1.0, 0.05)
...
config.gtInstancesFile = os.path.join(
...
def readPredInfo(predInfoFileName, config):
    ...
    with open(predInfoFileName, "r") as f:
        for line in f:
            ...
            if (
                os.path.commonprefix([filename, config.predictionPath])
                != config.predictionPath
            ):
                printError(
...
def matchGtWithPreds(predictionList, groundTruthList, gtInstances, config):
    ...
    for (pred, gt) in zip(predictionList, groundTruthList):
        ...
        (curGtInstances, curPredInstances) = assignGt2Preds(
            curGtInstancesOrig, gtImage, predInfo, config
        )
...
def computeAverages(aps, config):
    ...
    d50m = np.where(np.isclose(config.distanceThs, 50.0))
    d100m = np.where(np.isclose(config.distanceThs, 100.0))
...
def printResults(avgDict, config):
    ...
    for (lI, labelName) in enumerate(config.instLabels):
        ...
        if config.distanceAvailable:
            ...
            line += (
                getColorEntry(ap100m, config) + sep + "{:>15.3f}".format(ap100m) + sep
            )
            line += (
                getColorEntry(ap5050, config) + sep + "{:>15.3f}".format(ap5050) + sep
            )
        line += noCol
        print(line)
    ...
    if config.distanceAvailable:
        line += (
            getColorEntry(allAp50m, config) + sep + "{:>15.3f}".format(allAp50m) + sep
        )
        line += (
            getColorEntry(allAp100m, config) + sep + "{:>15.3f}".format(allAp100m) + sep
        )
        line += (
            getColorEntry(allAp5050, config) + sep + "{:>15.3f}".format(allAp5050) + sep
        )
    line += noCol
    print(line)
    print("")
...
def main(argv):
    ...
    if len(argv) >= 1:
        ...
        config.exportFile = os.path.join(
            config.predictionPath, "resultInstanceLevelSemanticLabeling.json"
        )
