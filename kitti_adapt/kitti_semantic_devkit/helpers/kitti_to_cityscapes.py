...
def kitti_to_cityscapes_instaces(instance_img):
    ...
    cs_instance = (
        kitti_semantic * 1000 + kitti_instance
    ) * instance_mask + kitti_semantic * (1 - instance_mask)
    return cs_instance
...
if __name__ == "__main__":
    ...
    semantic_file_list = [
        f for f in os.listdir(semantic_dir) if os.path.isfile(join(semantic_dir, f))
    ]
    ...
    for f in semantic_file_list[:4]:
        ...
        out_semantic_filename = join(
            out_semantic_dir, "kitti_%s_gtFine_labelIds.png" % f[:-4]
        )
